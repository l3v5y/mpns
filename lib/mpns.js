...
var IconicTile = function(options){
    return new PushMessage('iconic','1', 'token',options)
};
...
PushMessage.prototype.validate = function() {
    if (this.pushType != 'toast' && this.pushType != 'tile'&& this.pushType != 'iconic') {
...
function wrapValue(object, key, name) 
{
    if(object[key]==0)
    {
        return startTag(name) + escapeXml(object[key]) + endTag(name);    
    }
    return object[key] ? startTag(name) + escapeXml(object[key]) + endTag(name) : null;
}

function iconicToXml(options) {
    var type = 'Tile';
